...
def _load_ui_labels() -> dict:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f).get("task_panel", {})
    except Exception:
        return {}
...
class TaskPanel(QWidget):
    ...
    def __init__(self, config: AppConfig, parent=None):
        ...
        self._task_title_map = labels.get("task_titles", DEFAULT_TASK_TITLES)
    ...
    def _build_ui(self):
        ...
        self._task_order = [str(name) for name in self.config.tasks.keys()]
    ...
    def _build_task_card(self, task_name: str, trigger: TaskTriggerType) -> StableElevatedCardWidget:
        ...
        if trigger == TaskTriggerType.DAILY:
            ...
            # 失败间隔
            fail_value = SpinBox(card)
            fail_value.setRange(1, 999999)
            fail_value.setValue(60)
            fail_value.setSuffix(" 秒")
            fail_value.valueChanged.connect(self._auto_save)
            form.addRow(self._field_label("失败间隔", card), fail_value)
            widgets["failure_interval"] = fail_value
            ...
            # 优先级
            priority = SpinBox(card)
            priority.setRange(1, 100)
            priority.setValue(10)
            priority.valueChanged.connect(self._auto_save)
            form.addRow(self._field_label("优先级", card), priority)
            widgets["priority"] = priority
    ...
    def _parse_next_run_datetime(cls, text: str) -> QDateTime:
        ...
        if normalized is None:
            normalized = "2026-01-01 00:00:00"
    ...
    def _load_config(self):
        ...
        for task_name in self._task_order:
            ...
            if "interval_value" in widgets and "interval_unit" in widgets:
                ...
                fail = widgets.get("failure_interval")
                if isinstance(fail, SpinBox):
                    fail.setValue(max(1, task_cfg.failure_interval_seconds))
                pri = widgets.get("priority")
                if isinstance(pri, SpinBox):
                    pri.setValue(max(1, task_cfg.priority))
            ...
            if isinstance(next_run, QDateTimeEdit):
                next_run.setDateTime(self._parse_next_run_datetime(str(task_cfg.next_run)))
    ...
    def _auto_save(self):
        ...
        for task_name in self._task_order:
            ...
            if "interval_value" in widgets and "interval_unit" in widgets:
                ...
                fail = widgets.get("failure_interval")
                if isinstance(fail, SpinBox):
                    task_cfg.failure_interval_seconds = max(1, int(fail.value()))
                pri = widgets.get("priority")
                if isinstance(pri, SpinBox):
                    task_cfg.priority = max(1, int(pri.value()))
                start_edit = widgets.get("enabled_time_start")
                end_edit = widgets.get("enabled_time_end")
                start = "00:00:00"
                end = "23:59:59"
    ...
    def refresh_snapshots(self, snapshot):
        ...
        for item in items:
            ...
            if not name:
                continue
            widgets = self._task_widgets.get(name)
            if not widgets:
                continue
            next_run = widgets.get("next_run")
